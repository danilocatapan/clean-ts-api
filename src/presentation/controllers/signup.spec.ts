...
const makeSut = (): SutTypes => {
  ...
  const sut = new SignUnController(emailValidatorStub, addAccountStub)
  return {
    sut,
    emailValidatorStub,
    addAccountStub
  ...
  test('Should return 400 if password confirmation fails', () => {
    ...
    const httpRequest = {
      body: {
        ...
        email: 'any_email@mail.com',
        password: 'any_password',
        passwordConfirmaton: 'invalid_password'
      }
    }
    const httpResponde = sut.handle(httpRequest)
    expect(httpResponde.statusCode).toBe(400)
    expect(httpResponde.body).toEqual(new InvalidParamError('passwordConfirmaton'))
  })
